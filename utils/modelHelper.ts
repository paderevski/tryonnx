...
export async function getSqueezenetSession(sessionId: string) : Promise< ort.InferenceSession | undefined> {
	if (sessionId === "wasm") {
		const session = await ort.InferenceSession
		.create('./_next/static/chunks/pages/squeezenet1_1.onnx',
		{ executionProviders: ['wasm'], graphOptimizationLevel: 'all' })
		return session;
	} else if (sessionId === "webgl") {
		const session = await ort.InferenceSession
		.create('./_next/static/chunks/pages/squeezenet1_1.onnx',
...
export async function runSqueezenetModel(preprocessedData: any, session: ort.InferenceSession): Promise<[any, number] | undefined> {
	if (session) {
